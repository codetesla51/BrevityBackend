...
class PdfController extends Controller
{
  ...
  private function storeSummary(
    ...
    $pdf->Cell(0, 10, "Summary Analysis", 0, 1, "C");
    $pdf->Ln(10);
  ...
  public function downloadPDF($id)
  {
    try {
      ...
      $filePath = $pdf->summary_path;

      if (!Storage::exists($filePath)) {
        return response()->json(["message" => "File not found"], 404);
      }

      return Storage::download($filePath, $pdf->original_filename);
    } catch (\Exception $e) {
      report($e);
      return response()->json(["message" => "Failed to download PDF"], 500);
    }
  }
}
